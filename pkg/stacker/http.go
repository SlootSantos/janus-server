...
func SetThirdPartyAWSCredentials(w http.ResponseWriter, req *http.Request) {
	...
	lambdaARN := setupThirdPartyAccount(req.Context(), user.ThirdPartyAWS)
...
func setupThirdPartyAccount(ctx context.Context, thirdPartyCreds *storage.ThirdPartyAWS) string {
	...
	role, err := i.CreateRole(&iam.CreateRoleInput{
		...
		RoleName:                 aws.String("role-with-policy-11"),
	})
	log.Printf("%+v", role)
	log.Printf("%+v", err)

	pol, err := i.PutRolePolicy(&iam.PutRolePolicyInput{
